...
bot.on('text', async (ctx) => {
    ...
    if (text === '🇺🇿 O\'zbekcha') {
        ...
        return ctx.reply('Siz o\'zbek tilini tanladingiz.', Markup.keyboard.oneTime().resize());
    ...
    if (text === '🇷🇺 Русский') {
        userLanguage[chatId] = 'ru';
        return ctx.reply('Вы выбрали русский язык.', Markup.keyboard.oneTime().resize());
    ...
    if (text === 'Ha' || text === 'Да') {
        ...
        ctx.reply('Понятно, тогда Досвидос!');
    } else if (text === 'Нет') {
        ctx.reply('Понятно, тогда Досвидос!');
    }
});
